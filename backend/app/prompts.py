...
- pacing_ratio.target_total_duration_sec：全局目标总时长 = 所有组 target_duration_sec 之和
...
- pacing_ratio.target_total_duration_sec 必须等于所有组 target_duration_sec 之和
- 每组的 target_duration_sec 必须等于 shot_count × 平均单镜时长（action组约2s，铺垫组约4s）
...
6. **必须为每个分镜组输出终态描述（group_continuity）**，描述本组最后一帧的画面状态，供人工图生图和下一组分镜组生成时参考
