...
### synopsis（剧情概述）
- 必须 200 字以上，交代清楚本集完整叙事逻辑
- 包括：开场情境 → 核心冲突 → 高潮时刻 → 结尾悬念
- 重点描写**角色的具体动作和表情**，而非抽象概括

### key_events（关键剧情节点）
- **必须按时间顺序排列**（order 字段 = 1, 2, 3...）
- 每个节点必须包含：
  - description：具体发生了什么（包含角色名、动作、位置）
  - emotional_beat：该节点带给观众的情绪（激动/紧张/悲伤/震撼等）
- 节点数 = 分镜组数（每个关键事件 ≈ 一个分镜组的叙事核心）

### visual_highlights（视觉亮点）
- 描述本集最具视觉冲击力的 2-4 个画面时刻
- 每个亮点需包含：招式名、角色动作细节、特效描述、环境/光照
- 例如："萧炎右手玄重尺横扫，绿色异火沿尺身蔓延，周围空气被烧至扭曲，背景火光映红角色侧脸"

### emotional_arc（情绪走势）
- 描述本集情绪曲线，用箭头表示升跌，如：平静→紧张→爆发→短暂平息→最终高潮
- 标注每个情绪转折点的具体剧情事件

### storyboard_style_notes（分镜风格指导）
- 给出明确的运镜风格、色调基调、节奏快慢指导，不少于 100 字
- 例如："前三分之二慢节奏，营造压迫感；后三分之一快切，营造战斗张力"

### storyboard_shot_count（镜头数量）
- 参考值，仅用于前端显示；实际分镜由 Planner AI 根据剧情复杂度自行决定
- 如大纲中关键事件较多，Planner AI 会自动增加镜头数量

### estimated_duration_sec（预估时长）
- 参考值；实际总时长由分镜组数和每组镜头数决定
- 如剧情紧凑激烈，时长会偏短；抒情铺垫段落，时长会偏长

### 总体原则
- 充分吸收用户在对话中提出的所有修改意见
- 描述越具体越好：具体角色名 > 动作细节 > 环境/光照 > 情绪
...
- **重要：无角色的镜头（如纯环境镜头、空镜）必须将 characters_config 设为 null 或空列表 []，禁止写入任何描述文本**
...
### 图像生成提示词 — 中文
- image_prompt：**中文图像生成提示词**，包含画面主体描述、角色外貌特征（发色、瞳色、表情）、
  服装细节、动作姿态、特效描述（火焰/斗气颜色和形态）、环境光影（光源方向、色温）、
  镜头效果（景深、运动模糊）。中文描述更利于 AI 理解和中国风格一致性。
  示例："动漫风格，高质量，动态光照，萧炎面部特写，表情坚毅，黑色长发，
  绿色异火在眼中燃烧，汗水滴落，破旧衣衫，暖橙色逆光，背景为燃烧的宗门大殿"
- negative_prompt：英文负面提示词，如 "low quality, blurry, modern buildings, realistic photo, western fantasy"
...
- 无角色镜头（如纯环境/空镜）的 characters_config 必须为 null 或 []，禁止写入描述文字
...
class PlanPacingRatioSchema(BaseModel):
    ...
    target_total_duration_sec: int = Field(
        description="目标总时长（秒），必须等于所有镜头 duration_sec 之和"
    )
...
class ShotGroupPlanSchema(BaseModel):
    ...
    total_shot_count: int = Field(description="所有组分镜数之和，由 AI 根据剧情复杂度自主决定")
...
class ShotGroupFramePlan(BaseModel):
    ...
    image_prompt_for_generation: str = Field(
        description="中文图生图提示词，用于生成该分镜组的首帧图片"
    )
    negative_prompt: str = Field(
...
class ShotGroupPlanItemSchema(BaseModel):
    ...
    target_duration_sec: float = Field(
        description="本组分镜组目标时长（秒），须等于组内所有镜头 duration_sec 之和"
    )
    beats_per_second: float = Field(
        description="每秒剧情密度（关键动作/事件数），如 2.0 表示每秒 2 个关键事件"
    )
    key_visual_moments: List[str] = Field(
        description="2-4 个必须覆盖的具体视觉时刻，与原著情节锚定，如'萧炎右手斗气爆发特写'"
    )
    # 供 Phase 2 创作师参考的上一组尾帧描述（由 Planner AI 填写，组 1 此字段为空）
    prev_group_end_state: Optional[str] = Field(
        None,
        description=(
            "上一组分镜组结束时的画面状态描述（中文）。"
            "由 Planner AI 根据叙事顺序填写，组 1 此字段为空。"
            "Phase 2 创作师将此描述注入提示词，保证组间镜头叙事衔接。"
        ),
    )
...
class ShotDraftItemSchema(BaseModel):
    ...
    image_prompt: str = Field(description="中文图像生成提示词")
...
class DirectorAdjustmentSchema(BaseModel):
    ...
    group_continuity: List["ShotGroupContinuitySchema"] = Field(
        description=(
            "每个分镜组的终态描述（中文），供下一组生成时参考。"
            "顺序与 Phase 1 的 groups 一致。必须覆盖所有分镜组。"
        )
    )


class ShotGroupContinuitySchema(BaseModel):
    """Phase 3 导演输出的单个分镜组终态描述。"""
    group_code: str = Field(description="对应分镜组编号")
    end_frame_description: str = Field(
        description="中文描述本组最后一帧的画面状态，用于下一组生成时保持视觉连续"
    )
    end_character_states: List[str] = Field(
        description="关键角色在本组结束时的状态，如 ['萧炎：右手持尺，嘴角带血', '纳兰嫣然：剑尖下垂，不敢置信']"
    )
    end_lighting: str = Field(
        description="本组结束时的光照状态描述，如 '火光映照，主体轮廓镀金边'"
    )
...
### 时长与密度约束
每组必须精确设计：
- target_duration_sec：本组目标时长（秒），约等于组内镜头数 × 平均单镜时长（action组约2s/镜，铺垫组约4s/镜）
- beats_per_second：每秒剧情密度，如 2.0 = 每秒 2 个关键动作/事件

### 序号规范（关键）
- 第一组：sequence_start=1
- 第二组：sequence_start=上一组的 sequence_end+1
- 最后一组的 sequence_end = total_shot_count（必须等于总镜头数）
...
- image_prompt_for_generation：**中文图生图提示词**，用于生成该分镜组的首帧图片
...
- 四层视觉规划必须完整填写，不能为空
- **prev_group_end_state（组间衔接描述）**：从第 2 组开始，每组必须填写。
  描述上一组最后一帧的画面状态（角色位置/表情/光照/构图），供 Phase 2 创作师注入提示词，
  保证组间叙事衔接越连贯越好。越具体越好，例如：
  '上一组最后一镜：萧炎右手玄重尺拄地，口吐鲜血单膝跪地，背景火光映红，冷白色斗气从脚下蔓延'
  """
...
- **组间衔接描述（prev_group_end_state）**：上一组最后一帧的画面状态。
  **如果提供了此描述，你的第一镜必须从上一组的尾帧状态自然过渡，**
  不能有角色突然换位、表情突变、光照跳变等违和感。
...
### 图像生成提示词 image_prompt — 中文
- 主体：用中文详细描述画面内容，包含角色外貌特征（发色、瞳色、表情）、
  服装细节、动作姿态、特效描述（火焰/斗气颜色和形态）、环境光影（光源方向、色温）
...
6. **必须为每个分镜组输出 group_continuity（组终态描述）**，此描述非常关键：
   - group_code：对应的组编号（如 G001）
   - end_frame_description：**中文描述本组最后一帧的具体画面状态**（越具体越好）
     必须包含：角色的具体动作/表情/位置、背景/环境、光照色调
     示例："萧炎单膝跪地，右手撑玄重尺，嘴角流血，绿色异火从脚下向四周蔓延，
     背景火光映红侧脸，镜头低角度仰拍，肃杀压抑的冷白光"
     此描述将展示给用户，作为其生成下一组首帧图时的参考提示词
   - end_lighting（可选）：本组结束时的光照状态
   - end_character_states（可选）：关键角色在本组结束时的状态列表
...
def build_video_prompt(
    ...
    设计原则：图片驱动，文字只补图片无法传达的动态信息。
    已在图片中呈现的信息（构图、景别、角色外观、场景布局）不再重复。
    ...
    _append_line(lines, "入场", TRANSITION_MAP.get(transition_in, transition_in))

    transition_out = shot.transition_out or "cut"
    _append_line(lines, "出场", TRANSITION_MAP.get(transition_out, transition_out))

    # ── 光效（静态图片无法传达动态光照） ──
    lighting = environment.get("lighting")
    if lighting:
        # 加上 animated 关键词引导视频模型生成动态光效
        _append_line(lines, "光效", f"{lighting}，animated light effects, dynamic glow")

    # ── 特效动态 ──
    sfx_list = sound_design.get("sfx_list")
    if isinstance(sfx_list, list) and sfx_list:
        _append_line(lines, "特效", ", ".join(str(s) for s in sfx_list[:3]))

    # ── 情绪基调（语气，图片无法传达） ──
    tone = dialogue_delivery.get("tone")
    if tone:
        _append_line(lines, "情绪", tone)
...
def build_i2v_prompt(
    ...
    核心原则：只写图片/视频模型"看不到"的动态信息。
    图片已表达的静态信息（景别、构图层次、角色外观、场景布局）不再重复。

    必须包含（图片无法表达）：
      - camera.movement（运镜运动）
      - characters_config.action（角色动作动态）
      - sound_design.ambient / sfx_list（声音，图片看不到）
      - dialogue_delivery（台词节奏/语气，图片看不到）
      - atmosphere（氛围，影响视频调色和节奏）

    不写（图片已表达）：
      - shot_type / angle（景别机位已在画面中）
      - composition 层次细节（前景/中景/背景已在画面中）
      - lighting 静态描述（已在画面中）
      - 环境描述（已在画面中）
    ...
    if sfx_list:
        ...
        parts.append(f"音效:{sfx_str}")
    music = sound_design.get("music")
    if music:
        parts.append(f"音乐:{music}")

    # ── 台词演绎（图片看不到，文字也看不到，节奏必须写）──
    tone = dialogue_delivery.get("tone")
    ...
    if tone or pace:
        delivery_parts = []
        if tone:
            delivery_parts.append(f"语气:{tone}")
        if pace:
            delivery_parts.append(f"语速:{pace}")
        if isinstance(emotion_tags, list) and emotion_tags:
            delivery_parts.append(f"情绪标签:{','.join(emotion_tags[:3])}")
        parts.append("、".join(delivery_parts))
