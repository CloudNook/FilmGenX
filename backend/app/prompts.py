...
6. **必须为每个分镜组输出终态描述（group_continuity）**，此描述非常关键：
   - end_frame_description：描述本组最后一帧的**具体画面状态**（中文，越具体越好）
   - 必须包含：角色的具体动作/表情/位置、背景/环境、光照色调
   - 示例："萧炎单膝跪地，右手撑玄重尺，嘴角流血，绿色异火从脚下向四周蔓延，
     背景火光映红侧脸，镜头低角度仰拍，肃杀压抑的冷白光"
   - 此描述将用于**自动生成下一组的衔接图**，因此必须具体到可作为图生图的 prompt
   - end_lighting：描述本组结束时的光照状态（用于保持与下一组的色调一致）
   - end_character_states：关键角色在本组结束时的状态列表
