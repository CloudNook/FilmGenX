...
调 ``generate_video`` 时声音 / 字幕的约束是**非对称**的——声音可剥离允许出、字幕烧进像素禁止出：

**声音层（默认开启）**
- ``generate_audio`` **传 True**（默认值，不要传 False）
- 角色对白 / 人声**需要**——剧本里有台词的镜头，在 prompt 里写明角色说什么，让 Seedance 同步出对白音轨
- 环境音 / 动作音允许（风 / 雨 / 脚步 / 武器碰撞 / 火焰）
- BGM / 配乐建议留给后期——prompt 不主动要求 BGM / 鼓点 / 节拍

**视觉文字层（绝对禁止）**
- prompt 里**禁止**出现：字幕 / caption / 标题卡 / title card / 文字水印 / Logo / 角标 / 弹幕 / 文字标注
- 反例：``"画面下方出现字幕'萧炎登场'"`` / ``"标题卡：第一章 觉醒"`` —— 这些会让 Seedance 真的烧字进画面，整段镜头报废
- 例外：场景里的招牌 / 横幅 / 书页 / 手机屏幕显示的字——这些是**画面内容本身**，可以保留；字幕指的是"叠加在画面上的解说层"

简单记法：**声音可剥离 → 允许；字幕烧进像素 → 禁止**。原因：FilmGenX 产物是给后期人工剪辑用的，音视频可分离，但字幕一旦烧进像素就无法移除。
